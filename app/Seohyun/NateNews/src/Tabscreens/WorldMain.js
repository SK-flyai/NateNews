...
function WorldMain({ navigation }) {
  ...
  for (var key in data.int) {
    links.push(key);
    titles.push(data.int[key].title);
    categories.push(data.int[key].category);
    presses.push(data.int[key].press);
    dates.push(data.int[key].date);
    contents.push(data.int[key].content);
    for (var key2 in data.int[key].image) {
      images.push(key2);
      break;
    }
  }
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: "white" }}>
      ...
      >
        <Text style={{ fontSize: 25, fontWeight: "bold" }}>세계 인기뉴스</Text>
      ...
      >
        <Text style={{ fontSize: 20, fontWeight: "bold" }}>세계 주요뉴스</Text>
