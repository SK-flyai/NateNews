...
const CurrentContent = ({ route }) => {
  ...
  if (count == 0) {
    title = data.sisa[link].title;
    category = data.sisa[link].category;
    press = data.sisa[link].press;
    date = data.sisa[link].date;
    content = data.sisa[link].content;
    caption = data.sisa[link].caption;
  }
  for (var key3 in data.sisa[link].image) {
    img = key3;
    break;
  }
          ...
          <View key={i}>
            <TouchableOpacity
              onPress={() => {
                ...
                navigation.navigate("CurrentContent2", {
                  param1: modalLink[i],
