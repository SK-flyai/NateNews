...
function TotalMain({ navigation }) {
  ...
  for (var key in data.all) {
    links.push(key);
    titles.push(data.all[key].title);
    categories.push(data.all[key].category);
    presses.push(data.all[key].press);
    dates.push(data.all[key].date);
    contents.push(data.all[key].content);
    for (var key2 in data.all[key].image) {
      images.push(key2);
      break;
    }
  }
