...
function SocialMain({ navigation }) {
  const links = [];
  const titles = [];
  const categories = [];
  const presses = [];
  ...
  for (var key in data.soc) {
    links.push(key);
    titles.push(data.soc[key].title);
    categories.push(data.soc[key].category);
    presses.push(data.soc[key].press);
    dates.push(data.soc[key].date);
    contents.push(data.soc[key].content);
    for (var key2 in data.soc[key].image) {
      images.push(key2);
      break;
    }
  }
      ...
      <View styles={{ flex: 1 }} key={i}>
        ...
        <TouchableOpacity
          onPress={() => {
            ...
            navigation.navigate("SocialContent", links[i]);
          }}
        >
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: "white" }}>
      ...
      >
        <Text style={{ fontSize: 25, fontWeight: "bold" }}>사회 인기뉴스</Text>
      ...
      >
        <Text style={{ fontSize: 20, fontWeight: "bold" }}>사회 주요뉴스</Text>
